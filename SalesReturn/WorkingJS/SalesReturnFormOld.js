...
app.controller('SalesReturnController', function ($scope, HomeService, SalesReturnsService, RequestTypeService, $rootScope, $window, $filter,$q) {
    ...
    $scope.Update = function () {
        ...
        $scope.$API_Check().then(function (Obj) {
            ...
            angular.forEach($scope.Request.RequestDetail, function (value, key) {
                ...
                var batch = "";
                if (value.selectedSKU.BatchNoText != null && value.selectedSKU.BatchNoText != "") {
                    batch = value.selectedSKU.BatchNoText;
                }
                else {
                    batch = value.selectedSKU.Batch_No;
                }
                angular.forEach($scope.Request.RequestDetail, function (value1, key1) {
                    var batch1 = "";
                    if (value1.selectedSKU.BatchNoText != null && value1.selectedSKU.BatchNoText!="") {
                        batch1 = value1.selectedSKU.BatchNoText;
                    }
                    else {
                        batch1 = value1.selectedSKU.Batch_No
                    }
                    if (key != key1 && value1.InvoiceNo == value.InvoiceNo && value1.selectedSKU.SKUCode == value.selectedSKU.SKUCode && batch1 == batch) {
    ...
    $scope.Add = function () {
        console.log($scope.Request.RequestDetail);
