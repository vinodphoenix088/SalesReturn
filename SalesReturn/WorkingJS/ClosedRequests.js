...
app.service('ClosedRequestService', function ($http, $location, HomeService) {

    this.getClosedRequest = function (EmployeeCode) {

        return $http.get(HomeService.AppUrl + 'Closed/getClosedRequest?EmployeeCode=' + EmployeeCode, {});
    };
});



app.controller('ClosedRequestController', function ($scope, HomeService, RequestTypeService, $uibModal, PendingRequestService, ClosedRequestService, $rootScope) {
    ...
    $scope.init = function () {
        ...
        ClosedRequestService.getClosedRequest($rootScope.session.EMP_CODE).then(function (success) {
